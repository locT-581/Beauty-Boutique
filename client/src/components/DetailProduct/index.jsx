    ...
    try {
      ...
      const promises = mainQuerySnapshot.docs.map(async (mainDoc, index) => {
        ...
        subQuerySnapshot.forEach((subDoc) => {
          console.log(subDoc.id, currentProduct.category);
    ...
    <main className="">
      <div className="px-[5%] mx-[10%] mt-10 ">
        <Breadcrumbs aria-label="breadcrumb">
          <Link underline="hover" color="inherit" href="/trangchu">
            Trang chủ
          </Link>
      ...
      <section className="flex px-[5%] mx-[10%] mb-10">
        ...
        <div className="w-6/12 px-3 flex flex-col justify-between pb-10">
          ...
          <div>
            ...
            {currentProduct.ingredients?.map((ingredient, index) => (
              <div key={index} className="ml-4">
                ...
                <span className="text-black ml-4">x{ingredient.quantity}</span>
              </div>
            ))}
          </div>
          <p className="text-[12px]">
            *Lưu ý: Hình ảnh sản phẩm chỉ mang tính chất thiết kế tham khảo. Vì
            các loại hoa sẽ được thay đổi theo ngày, theo mùa, chúng tôi khuyến
            khích khách hàng ĐỂ LẠI LỜI NHẮN, để nhận được tư vấn hoa phù hợp
            theo nhu cầu của quý khách.
          </p>
          <div className="flex justify-around px-3">
            <Button className="w-[40%] mx-auto py-3" color="black">
              Thêm vào giỏ hàng
            </Button>
            <Button className="w-[45%] mx-auto" color="pink">
              Mua ngay
            </Button>
          </div>
          ...
          <div>
            ...
            <MailOutlineIcon className="mx-2 cursor-pointer" fontSize="small" />
            <LocalPhoneIcon className="mx-2 cursor-pointer" fontSize="small" />
          </div>
        </div>
      </section>
